...
echo -e "${YELLOW}Corrigindo problemas com o tema escuro...${NC}"
...
# Verificar se há referências ao Material UI e remover
echo -e "${GREEN}Verificando referências ao Material UI...${NC}"

# Remover referências ao Material UI em _app.tsx se existir
if [ -f "src/pages/_app.tsx" ]; then
    echo -e "${GREEN}Atualizando src/pages/_app.tsx...${NC}"
    # Substituir ErrorBoundary por ErrorBoundaryTailwind
    sed -i 's/import ErrorBoundary from/import ErrorBoundaryTailwind from/g' src/pages/_app.tsx
    sed -i 's/import { ThemeProvider } from ..\\/contexts\\/ThemeContext/import { ThemeProvider } from "next-themes"/g' src/pages/_app.tsx
    sed -i 's/<ErrorBoundary>/<ErrorBoundaryTailwind>/g' src/pages/_app.tsx
    sed -i 's/<\\/ErrorBoundary>/<\\/ErrorBoundaryTailwind>/g' src/pages/_app.tsx
fi

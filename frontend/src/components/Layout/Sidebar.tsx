    ...
    <div className={`flex flex-col h-full bg-white dark:bg-gray-800 shadow-lg transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'} p-4`}>
      <div className="flex items-center h-16 mb-4">
        <div className="flex items-center">
          <Image src="/images/iconeuniatende.png" alt="Logo Unigate" width={36} height={36} />
          {!collapsed && (
            <span className="ml-2 text-xl font-bold text-primary dark:text-white">Unigate</span>
          )}
        </div>
        <button
          onClick={() => setCollapsed((c) => !c)}
          className="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors ml-auto"
